...
// import requestService from '../services/requestService'; // adjust path if needed
...
//       // Load open maintenance request count for this equipment
//       const res = await requestService.getAllRequests({ equipmentId: id, openOnly: true });

//       // handle {success:true, data:[...]} or direct array
//       const payload = res?.data ?? res;
//       const list = payload?.data ?? payload;

//       setOpenReqCount(Array.isArray(list) ? list.length : 0);
//     } catch (e) {
//       console.error('Failed to load equipment/count', e);
//       setOpenReqCount(0);
//     } finally {
//       setLoading(false);
//     }
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // 2) Load open maintenance request count for this equipment
        ...
        // some services might already return "data", so handle both shapes
        const payload = res?.data ?? res;
        const list = payload?.data ?? payload;
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ed-requests-btn { width: 100%; background: #1a1a1a; border: 1px solid #2a2a2a; color: #fff; font-family: 'Barlow Condensed', sans-serif; font-size: 15px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; padding: 16px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; }
        .ed-requests-btn:hover { border-color: #f0a500; color: #f0a500; }
        .ed-badge-count { background: #f0a500; color: #000; font-size: 12px; font-weight: 700; padding: 2px 8px; border-radius: 10px; min-width: 24px; text-align: center; }
        @media (max-width: 768px) { .ed-grid { grid-template-columns: 1fr; } .ed-info-grid { grid-template-columns: 1fr; } }
      `}</style>
      ...
      <div className="ed-root">
        <div className="ed-header">
          <div className="ed-header-left">
            <button className="ed-back" onClick={() => (window.location.href = '/equipment')}>
              ← Back
            </button>
            <div className="ed-title">
              Equipment <span>Details</span>
            </div>
          </div>
          <div className="ed-btn-row">
            <button className="ed-btn" onClick={() => (window.location.href = `/equipment/edit/${id}`)}>
              Edit
            </button>
            <button className="ed-btn-del" onClick={handleDelete}>
              Delete
            </button>
          </div>
        </div>
        ...
        <div className="ed-grid">
          ...
          <div>
            <div className="ed-card" style={{ marginBottom: 24 }}>
              ...
              <div className="ed-info-grid">
                <div className="ed-field">
                  <div className="ed-field-label">Serial Number</div>
                  <div className="ed-field-value" style={{ fontFamily: 'monospace' }}>
          ...
          <div>
              ...
              <div className="ed-field">
                ...
                <span
                  ...
                  style={{
                    ...
                    border: `1px solid ${statusColor(equipment.status)}`,
                  }}
                >
                  {equipment.status}
                </span>
            ...
            <div className="ed-card">
              ...
              {/* ✅ Smart button */}
              <button
                className="ed-requests-btn"
                onClick={() => (window.location.href = `/requests?equipmentId=${id}`)}
                title="View open maintenance requests for this equipment"
              >
                View Maintenance Requests
              </button>
